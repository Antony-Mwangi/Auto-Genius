...
export default function RegisterPage() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const success = register({
      ...
      email,
      password,
      role: "customer", // ✅ FIX: required field added
    });
    ...
    // go to login first (cleaner flow)
    router.push("/login");
  };
    ...
    <div className="min-h-screen grid md:grid-cols-2 bg-[#0b0f14] text-white">
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <input
              ...
              onChange={(e) => setEmail(e.target.value)}
              required
              className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-teal-400"
            />
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-xl bg-teal-500 py-3 font-semibold hover:bg-teal-400 disabled:opacity-50"
            >
              {loading ? "Creating Account..." : "Create Account"}
            </button>
          </form>
      ...
      <div className="hidden md:flex flex-col justify-center bg-gradient-to-br from-orange-500/10 to-cyan-500/10 p-16">
        ...
        <h1 className="text-5xl font-bold">
          Welcome to <span className="text-orange-500">AutoSpare</span>
        </h1>

        <p className="mt-6 text-gray-300">
          Shop genuine spare parts and manage your orders easily.
        </p>
